...
namespace ArtificialIntelligenceEditor
{
    public partial class Artificial_Intelligence_Editor : Form
    {
        ...
        public Artificial_Intelligence_Editor()
        {
            ...
            LearnRateTextBox.Text = LearnRate.ToString();    //Tlačítka a texty
            Momentum = 0;
            MomentumTextBox.Text = Momentum.ToString();
            Epoch_textBox.Text = EpochCount.ToString();
            SigmoidKonstant = 1;
            ...
            neural_network = new NeuralNetwork(brImages, brLabels,ErrorChart);
            MainPanel.Paint += MainPanel_Paint;
        }

        void InitializeNeurons()
        {
            LayersNeurons = new int[2];
            LayersNeurons[0] = 2;
            LayersNeurons[1] = 1;
        }
        ...
        void InitializeWeights()
        {
            ...
            Weights = new double[LayersNeurons.Length - 1][][];   //Weights and Bias
            ...
            for (int a = 0; a < LayersNeurons.Length - 1; a++)
            {
                ...
                for (int b = 0; b < LayersNeurons[a + 1]; b++)
                {
                    ...
                    for (int c = 0; c < LayersNeurons[a]; c++)
                    {
                        ...
                        /*
                        double average = (LayersNeurons[a] + LayersNeurons[a + 1]) / 2;
                        double max = 1 / average;
                        double w=rnd.NextDouble()*max;

                        while (w==0)
                        {
                            max = 1 / average;
                            w = rnd.NextDouble()*max;
                        }
                        Weights[a][b][c] = w;   //Xavier inicialization */
                    }
                }
            }
        }
        ...
        void MNIST_image(Panel Mnist_Panel, int image_number, double[][] Dataset, double[][] Correct, bool Out)
        {
            ...
            if (bp == false)
            {
                NeuralNetwork neural_network2 = new NeuralNetwork(brImages, brLabels, ErrorChart);
        ...
        {
            LearnRate= double.Parse(LearnRateTextBox.Text);
            EpochCount = int.Parse(Epoch_textBox.Text);
            SigmoidKonstant = double.Parse(SigmoidTextBox.Text);
            Momentum = double.Parse(MomentumTextBox.Text);
            ...
            {
                neural_network.MNIST_LoadDataset(out GlobalVariables.Dataset, out GlobalVariables.Correct, MNIST_data); //785
            }
            ...
            neural_network.Backpropagation(Dataset, Correct, EpochCount, Momentum);
        ...
        private void InputComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                LearnRate = 60;                                 //Good start options               
                LearnRateTextBox.Text = LearnRate.ToString();
                ...
                NeuronCountTextBox.Text = string.Join(", ", LayersNeurons.Select(o => o.ToString()).ToArray());
                InitializeWeights();
            ...
            if ((string)InputComboBox.SelectedItem == "Čísla 0-9")
            {
                ...
                neural_network.Numbers_Load(out GlobalVariables.Dataset, out GlobalVariables.Correct,out original_pictures);
                int[] x = { 10000, 10 };
                LayersNeurons = x;
                NeuronCountTextBox.Text = string.Join(", ", LayersNeurons.Select(o => o.ToString()).ToArray());
                InitializeWeights();
        ...
        {
            LoadData(out GlobalVariables.Weights, out GlobalVariables.Bias,out GlobalVariables.LayersNeurons);
            /*
